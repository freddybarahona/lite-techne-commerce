...
import { SeedRoles } from "../../infrastructure/seed/seed.roles";
...
export default class serverconfigurations{
    ...
    private config_back({portBack}: {portBack: number}){
      this.back.app.listen(portBack,() => {
        console.log(InitializeConstants.infoBackActive({port:portBack, ms_name:"auth"}))
      })
    }
}